...
const STORAGE_KEY = "sih_final_round_selected_team_ids_v1";
...
export function AdminFinalRoundSelection() {
  ...
  const [activeTab, setActiveTab] = useState("SELECTOR"); // "SELECTOR", "SHOWCASE", "PRINT_CIRCULAR"
  ...
  const [customCircularDate, setCustomCircularDate] = useState(() => new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" }));
  ...
  // Teams that are selected for final round
  const selectedTeams = useMemo(() => {
    const selectedSet = new Set(selectedTeamIds);
    return confirmedTeams.filter((t) => selectedSet.has(t.id));
  }, [confirmedTeams, selectedTeamIds]);

  // Toggle selection for a single team
  ...
  function handleExportSelectedCsv() {
    ...
    downloadCsv(`SIH2026_Top_${selectedTeams.length}_Final_Round_Selected_Teams.csv`, rows);
  }

  // Copy WhatsApp / Telegram Announcement Broadcast message
...
- SIH Organizing Committee & Jury Panel`;
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="no-print space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-gold px-3.5 py-1 text-xs font-black text-web uppercase tracking-wider shadow-xs">
              ...
              <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-black uppercase tracking-wider ${
                ...
                <CheckCircle2 size={13} /> {selectedTeams.length} / {targetLimit} Teams Selected
              </span>
            </div>
            <h1 className="font-display text-3xl sm:text-4xl text-web flex items-center gap-2.5">
              Final Round Selected Teams
            </h1>
            <p className="text-xs sm:text-sm font-semibold text-ink/75 max-w-2xl">
              Manually click checkboxes on candidate teams to select the Top 25 Finalists. Instantly generate celebratory Congratulations announcements and high-resolution official PDF circulars.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              title="Copy formatted WhatsApp announcement message"
            >
              {copiedBroadcast ? <Check size={14} /> : <Share2 size={14} />}
              {copiedBroadcast ? "Copied Message!" : "WhatsApp Broadcast"}
            </button>
            ...
            >
              <FileSpreadsheet size={14} className="text-gold" /> Export 25 CSV
            </button>
        ...
        {/* Quick Target Progress & Metrics Bar */}
        <div className="rounded-3xl border-3 border-web bg-gradient-to-r from-web via-slate-900 to-web p-5 text-white shadow-comic">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Flame className="text-gold" size={20} />
                <h3 className="font-display text-xl text-gold">
                  Final Round Selection Progress
                </h3>
              </div>
              <p className="text-xs text-white/80 font-medium">
                {selectedTeams.length < targetLimit 
                  ? `Select ${targetLimit - selectedTeams.length} more teams to complete the Top ${targetLimit} Grand Finale roster.`
                  : selectedTeams.length === targetLimit
                  ? `🎉 Perfect! Exactly ${targetLimit} teams are selected for the Grand Finale round.`
                  : `⚠️ ${selectedTeams.length} teams selected (${selectedTeams.length - targetLimit} more than target ${targetLimit}).`}
              </p>
            </div>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-wider mb-1">
              <span className="text-gold">{selectedTeams.length} Finalists Selected</span>
              <span className="text-white/70">{Math.round((selectedTeams.length / targetLimit) * 100)}% Complete</span>
            </div>
            <div className="h-3 w-full rounded-full bg-white/20 overflow-hidden p-0.5">
              <div 
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-slate-200 pb-3">
          <div className="flex bg-slate-200 p-1 rounded-2xl border-2 border-web/20">
            <button
            ...
            >
              <CheckCircle2 size={15} /> 1. Manual Selector ({selectedTeams.length})
            </button>
            ...
            >
              <Sparkles size={15} /> 2. Congratulations Showcase 🎉
            </button>
            ...
            >
              <Printer size={15} /> 3. Official PDF & Print Circular
            </button>
          </div>
        ...
        {/* TAB 1: MANUAL SELECTION & CHECKBOX GRID */}
        {activeTab === "SELECTOR" && (
          <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 rounded-2xl border-2 border-web/20 bg-white p-3 shadow-xs">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by team name, leader name, reg ID, problem statement..."
                  value={search}
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-web text-white font-black uppercase text-[10px] tracking-wider">
                    <tr>
                      ...
                      <th className="p-3.5">Status / Rank</th>
                      <th className="p-3.5">Team & Reg ID</th>
                      <th className="p-3.5">Problem / Project Title</th>
                      ...
                      return (
                        <tr
                          ...
                          className={`cursor-pointer transition select-none ${
                            isSelected
                              ? "bg-amber-50/80 hover:bg-amber-100/90 border-l-4 border-l-gold"
                              : "hover:bg-slate-50"
                          }`}
                        >
                          {/* Checkbox */}
                          ...
                          {/* Status / Finalist Rank Badge */}
                          <td className="p-3.5">
                            {isSelected ? (
                              <span className="inline-flex items-center gap-1 rounded-full bg-gold px-2.5 py-1 text-[11px] font-black text-web border border-web/20 shadow-xs">
                                <Crown size={12} className="text-spidey" /> Finalist #{selectedIndex + 1}
                              </span>
                            ) : (
        ...
        {/* TAB 2: CONGRATULATIONS SHOWCASE (Interactive Live View / Projector View) */}
        {activeTab === "SHOWCASE" && (
          <div className="space-y-6">
            {/* Grand Celebratory Banner */}
            <div className="relative overflow-hidden rounded-3xl border-4 border-gold bg-gradient-to-br from-slate-950 via-web to-slate-900 p-6 sm:p-8 text-center text-white shadow-2xl">
              <div className="relative z-10 max-w-3xl mx-auto space-y-3">
                <div className="inline-flex items-center gap-2 rounded-full bg-gold/20 border border-gold/40 px-4 py-1 text-xs sm:text-sm font-black text-gold uppercase tracking-widest shadow-inner">
                  <Sparkles size={16} className="text-gold animate-spin" /> SMART INDIA HACKATHON 2026
                </div>
                ...
                <p className="text-xs sm:text-sm text-slate-300 font-semibold max-w-xl mx-auto">
                  Selected through rigorous jury evaluation. All qualified teams must report to the grand stage for the final presentation round.
                </p>
              ...
              {!selectedTeams.length && (
                <div className="col-span-full rounded-3xl border-3 border-dashed border-slate-300 p-12 text-center bg-white">
                  ...
                  <p className="text-xs text-slate-400 font-bold mt-1">
                    Switch to the "Manual Selector" tab and check the boxes next to the candidate teams.
                  </p>
                </div>
              )}
            </div>
        ...
        {/* TAB 3: OFFICIAL PRINT & PDF CIRCULAR PREVIEW */}
        {activeTab === "PRINT_CIRCULAR" && (
          <div className="space-y-4">
            {/* Print Controls Bar */}
...
// Subcomponent: Clean, Official Circular Document for Screen and Print
function OfficialCircularDocument({ selectedTeams, circularRef, circularDate }) {
  return (
    <div className="space-y-4 text-black text-left font-sans">
      ...
      <div className="border-b-2 border-black pb-3">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="h-16 w-16 bg-slate-900 rounded-xl flex items-center justify-center p-2 text-white font-black text-center text-xs">
              SIH 2026
            </div>
            <div>
              <h1 className="font-serif text-lg sm:text-xl font-black uppercase tracking-tight text-slate-950">
                GRAMIN TECHNICAL & MANAGEMENT CAMPUS, NANDED
              </h1>
              <p className="text-[10px] font-bold text-slate-700">
                Approved by AICTE, New Delhi | Affiliated to SRTMU Nanded & MSBTE Mumbai
              </p>
              <p className="text-[11px] font-black text-spidey uppercase mt-0.5">
                SMART INDIA HACKATHON 2026 — INTERNAL ROUND & GRAND FINALE
              </p>
            </div>
          </div>
          <div className="text-right">
            <span className="inline-block border border-black px-2.5 py-1 text-[9px] font-mono font-black uppercase">
      ...
      <div className="flex items-center justify-between text-xs font-bold border-b border-slate-300 pb-2">
      ...
      <div className="text-center py-1">
        <h2 className="text-sm sm:text-base font-black uppercase tracking-wider text-black border-y-2 border-black py-1.5 bg-slate-100">
          OFFICIAL NOTIFICATION: SHORTLISTED TEAMS FOR THE FINAL ROUND / GRAND FINALE
        </h2>
      </div>

      {/* Congratulatory preamble */}
      <div className="text-xs text-justify leading-relaxed text-slate-800 bg-amber-50/50 p-2.5 border border-amber-300 rounded">
        <p>
      ...
      {/* High-Resolution Selected Teams Table */}
